...
results_tbl_EXP2[idx,]$perc_dist = 0.01

results_tbl_EXP2$WC = "IC" 

subjects_to_remove <- c("S006","S009", "S013", "S018", "S019","S024")
idx = results_tbl_EXP2$subject == subjects_to_remove[1]
results_tbl_EXP2[idx,]$WC = "WC"
idx = results_tbl_EXP2$subject == subjects_to_remove[2]
results_tbl_EXP2[idx,]$WC = "WC"
idx = results_tbl_EXP2$subject == subjects_to_remove[3]
results_tbl_EXP2[idx,]$WC = "WC"
idx = results_tbl_EXP2$subject == subjects_to_remove[4]
results_tbl_EXP2[idx,]$WC = "WC"
idx = results_tbl_EXP2$subject == subjects_to_remove[5]
results_tbl_EXP2[idx,]$WC = "WC"
idx = results_tbl_EXP2$subject == subjects_to_remove[6]
results_tbl_EXP2[idx,]$WC = "WC"
...
# Todos los sujetos
cbPalette <- c( "#009E73", "black","#999999", "#D55E00", "#F0E442")
FittedlmPlot1 <-ggplot(data = filter(results_tbl_EXP2,condition == "Incongruous room") , aes(x = target_distance, y = 10^Modelfitted3, group = subject, color = WC))+
  geom_line()+
  geom_abline(intercept = 0, slope = 1, linetype=3) +
  scale_colour_manual(values = cbPalette) + 
  scale_fill_manual(values = cbPalette) + 
...
ggsave(mi_nombre_de_archivo, plot = FittedlmPlot1, width=10, height=10, units="cm", limitsize=FALSE, dpi=200)
...
results_tbl_dis_mean = merge(results_tbl_dis_mean_ir,results_tbl_dis_mean_nir, all = TRUE)
m.disError <- lm(Merror ~ condition, 
                   data = results_tbl_dis_mean)
extract_stats(ggcoefstats(m.disError))
anova(m.disError)

results_tbl_dis_mean$condition <- factor(results_tbl_dis_mean$condition, levels = c("Congruous room", "Incongruous room"))
cbPalette <- c("#81D4FA","#009E73","#E0F7FA", "#D55E00", "#CC79A7", "#F0E442")
fig.error_s <- ggplot(results_tbl_dis_mean, aes(x = condition,
  ...
  scale_colour_manual(values = cbPalette) + 
  scale_fill_manual(values = cbPalette) +
  scale_x_discrete(name="Condition",labels = c("Exp.1","Exp.2"))+
  scale_y_continuous(name="% Error discrimination",  limits = c(0,120)) +
  expand_limits(y = c(0.5, 13.5))+
  ggtitle("Stimuli via speaker")+
  annotate("text", x = 1.5, y = 116,  label = "***", size = 5) +
  annotate("segment", x = 1, xend = 2, y = 114, yend = 114, colour = "black", size=.3, alpha=1,)+
  theme_pubr(base_size = 10, margin = TRUE)+
...
fig.error_h <- ggplot(results_tbl_dis_mean, aes(x = condition,
  ...
  geom_point(alpha = 0.4,
             position = position_jitterdodge(jitter.width = .3,
                                             jitter.height = 0,
                                             dodge.width = 1 )) +
  scale_shape_manual(values=c(15, 17, 23, 23))+
  geom_abline(slope = 0, 
              intercept = 0, 
              alpha = 0.5, 
              linetype = "dashed") +
  ...
  scale_colour_manual(values = cbPalette) + 
  scale_fill_manual(values = cbPalette) +
  scale_x_discrete(name="Condition",labels = c("I-CR","NI-CR",
                                                       "NI-IR"))+
